...
export const EventForm = ({ onSuccess, onCancel }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        paymentMethod: 'none', 
        price: '', discountDetails: '', paymentDetails: '', 
        photoUrl: '', logoUrl: '' 
    });
            ...
            <form onSubmit={handleSubmit} className="event-form" noValidate>
                ...
                <div style={{ display: 'flex', gap: '15px', marginTop: '10px' }}>
                    <div className="form-group" style={{ flex: 2 }}>
                        <label>תאריך </label>
                        <input type="date" min={todayStr} value={formData.date} onClick={(e) => e.target.showPicker?.()} onChange={(e) => setFormData({...formData, date: e.target.value})} className={`input-standard standard-numbers ${errors.date ? 'error-border' : ''}`} />
                        {errors.date && <span className="error-text">{errors.date}</span>}
                    </div>
                    <div className="form-group" style={{ flex: 1 }}>
                        <label>התחלה </label>
                        <input type="time" value={formData.startTime} onClick={(e) => e.target.showPicker?.()} onChange={(e) => setFormData({...formData, startTime: e.target.value})} className={`input-standard standard-numbers ${errors.startTime ? 'error-border' : ''}`} />
                        {errors.startTime && <span className="error-text">{errors.startTime}</span>}
                    </div>
                    <div className="form-group" style={{ flex: 1 }}>
                        <label>סיום </label>
                        <input type="time" value={formData.endTime} onClick={(e) => e.target.showPicker?.()} onChange={(e) => setFormData({...formData, endTime: e.target.value})} className={`input-standard standard-numbers ${errors.endTime ? 'error-border' : ''}`} />
                    ...
                    <div className="panel-box">
                        ...
                        <div className="form-group" style={{ marginTop: '10px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                ...
                                {/* Fixed accessibility copy button to look like a pill */}
                ...
                <div className="form-group">
                    <label>אפשרות תשלום:</label>
                    <div style={{ display: 'flex', gap: '15px' }}>
                        <label className="radio-label">
                            <input type="radio" value="none" checked={formData.paymentMethod === 'none'} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value, price: '', paymentDetails: ''})} /> חינם / ללא תשלום
                        </label>
                        <label className="radio-label">
                            <input type="radio" value="link" checked={formData.paymentMethod === 'link'} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value, paymentDetails: ''})} /> קישור הרשמה
                        </label>
                        <label className="radio-label">
                            <input type="radio" value="bit" checked={formData.paymentMethod === 'bit'} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value, paymentDetails: ''})} /> ביט / Bit
                        </label>
                        <label className="radio-label">
                ...
                {formData.paymentMethod !== 'none' && (
                    <div className="panel-box">
                        <div style={{ display: 'flex', gap: '15px' }}>
                            ...
                            <div className="form-group" style={{ flex: 2 }}>
                                <label>הנחות</label>
                                <input type="text" value={formData.discountDetails} onChange={(e) => setFormData({...formData, discountDetails: e.target.value})} className="input-standard" placeholder="לדוגמה: 20% הנחה למשרתי מילואים" />
                            </div>
                        </div>

                        <div className="form-group" style={{ marginTop: '10px' }}>
                            {formData.paymentMethod === 'link' && <label>קישור לתשלום/הרשמה </label>}
                            {formData.paymentMethod === 'bit' && <label>מספר טלפון לביט </label>}
                            {formData.paymentMethod === 'other' && <label>הוראות תשלום </label>}
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '40px' }}>
                    ...
                    <button type="submit" className="btn-primary pill-btn" disabled={isLoading}>שמירה ופרסום</button>
                </div>
            </form>

            {/* MAP & WARNING MODALS (Unchanged logic, just ensure button has pill class) */}
            {isMapExpanded && (
